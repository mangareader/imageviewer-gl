...
import com.gbnix.imageviewer.util.MediaSetUtils;
...
public final class DataSourceType {
	...
	public static int identifySourceType(final MediaSet set) {
		if (set == null) return TYPE_NOT_CATEGORIZED;

		final Path path = set.getPath();
		if (MediaSetUtils.isCameraSource(path)) return TYPE_CAMERA;

		final Path prefix = path.getPrefixPath();
