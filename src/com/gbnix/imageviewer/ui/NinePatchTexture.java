...
public class NinePatchTexture extends ResourceTexture {
	...
	private final SmallCache<NinePatchInstance> mInstanceCache = new SmallCache<NinePatchInstance>();
	...
	private static class SmallCache<V> {
		...
		public V put(final int key, final V value) {
			if (mCount == CACHE_SIZE) {
				final V old = mValue[CACHE_SIZE - 1]; // remove the last item
